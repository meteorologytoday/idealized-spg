...
# (lat/lon both vary along *both* native dims in the SPG region -- this is a
# real distortion of the grid, not rounding noise), so its u/v are regridded
# onto a regular lat-lon grid first via _regrid_curvilinear_to_regular. This is
# the one place in this project interpolation is unavoidable: there is no way
# to draw streamlines on a curvilinear mesh directly. Note this regridding is
# purely for this visualization -- vozocrtx/vomecrty themselves are already
...
def _regrid_curvilinear_to_regular(lat, lon, data, lon_range, lat_range, resolution):
    """
    Linearly interpolate `data` from a curvilinear (2D lat/lon) grid onto a
    regular lat-lon grid spanning `lon_range`/`lat_range` at `resolution`
    degrees, as if generated by `np.meshgrid` -- the grid shape `streamplot`
    requires and a curvilinear NEMO grid does not natively provide.

    Cropping to the target region (padded by one grid cell) before handing
    points to scipy keeps the interpolation cheap and avoids pulling in
    points from clear across the curvilinear grid -- e.g. past the NEMO
    north fold -- that have nothing to do with the target region.

    lat, lon, data: 2D arrays on the curvilinear source grid, same shape
    lon_range, lat_range: (min, max) bounds of the target grid, in degrees
    resolution:  spacing of the target grid, in degrees

    Returns (grid_lon, grid_lat, regridded): 1D target-grid coordinate
    arrays and the 2D interpolated field (NaN outside the source data's
    convex hull).
    """
    pad = resolution
    in_region = (
        (lat >= lat_range[0] - pad) & (lat <= lat_range[1] + pad) &
        (lon >= lon_range[0] - pad) & (lon <= lon_range[1] + pad)
    )

    grid_lon = np.arange(lon_range[0], lon_range[1] + resolution / 2, resolution)
    grid_lat = np.arange(lat_range[0], lat_range[1] + resolution / 2, resolution)
    mesh_lon, mesh_lat = np.meshgrid(grid_lon, grid_lat)

    points = np.column_stack([lon[in_region], lat[in_region]])
    regridded = griddata(points, data[in_region], (mesh_lon, mesh_lat), method='linear')

    return grid_lon, grid_lat, regridded
...
def _plot_surface_current_panel(ax, dataset, years_list, season, lon_range, lat_range):
    ...
    if u['lat'].ndim == 2:
        # Curvilinear native grid (e.g. ORAS5's NEMO grid) -- regrid onto a
        # regular lat-lon grid first, since streamplot cannot draw on one
        # directly (see SURFACE_CURRENT_PANELS comment above).
        lon_plot, lat_plot, u_plot = _regrid_curvilinear_to_regular(
            u['lat'].values, u['lon'].values, u.values, lon_range, lat_range, STREAMPLOT_GRID_RESOLUTION)
        _, _, v_plot = _regrid_curvilinear_to_regular(
            v['lat'].values, v['lon'].values, v.values, lon_range, lat_range, STREAMPLOT_GRID_RESOLUTION)
    else:
        lon_plot, lat_plot, u_plot, v_plot = u['lon'].values, u['lat'].values, u.values, v.values

    speed = np.hypot(u_plot, v_plot)
    strm = ax.streamplot(lon_plot, lat_plot, u_plot, v_plot,
